...
data and data1 are two strings with rainfall records of a few 
cities for months from January to December. The records of 
...
if functions mean or variance have as parameter town a 
city which has no records return -1
...
depending on the language (see function assertFuzzyEquals).
...
*/
function mean(city, str){
  let cities = str.split("\n");
  cities.forEach(city => {

    let cityObj = city.split(":");
    console.log(cityObj[0] + " " + city);
    console.log(cityObj[0]==city);
    let cityRainfall = cityObj[1].split(",");
    //console.log(cityRainfall);
    
    if (cityObj[0].toString()==city){
      console.log(city);
      let sum = 0;
      monthRainfall.split(" ").forEach(monthInfo => {
        sum += monthInfo[1];
      });
      let mean = sum / cityRainfall.length;
...
function process(str){
  ...
  let lastCityNameBeginsAt = 0;
  let lastCityNameEndsAt =0;

  for(let i=0, max=str.length; i<max; i++){
    if(str[i]==='\n'){

      lastCityNameBeginsAt=i+1;

      // array of rainfall in month order starting with jan
      let monthlyValues = [];
      str.slice(lastCityNameEndsAt+1,i).split(',').forEach(monthObj => {
        monthlyValues.push((monthObj.split(' ')[1]));
      });
      
      allCities.push({ name: currentCity, rainfall: monthlyValues});
    ...
    if(str[i]===':') {
      lastCityNameEndsAt = i;
      currentCity = str.slice(lastCityNameBeginsAt,lastCityNameEndsAt);
    }
