...
function blackOrWhiteKey(keyPressCount) {
    ...
    function within88(keycount){
      return (keycount > 88) ? keycount % 88 : keycount;
    }
    function within12(keycount){
      return(keycount > 12) ? keycount % 12 : keycount;
    }
    function exactIn12(keycount){
      return (keycount ==0) ? 11 : keycount - 1; 
    }
    
    let nwithin88 = within88(keyPressCount);
    let nwithin12 = within12(nwithin88);
    let exact = exactIn12(nwithin12);
...
assert.equal(blackOrWhiteKey(1), "white")
assert.equal(blackOrWhiteKey(5), "black")
assert.equal(blackOrWhiteKey(12), "black")
assert.equal(blackOrWhiteKey(42), "white")
assert.equal(blackOrWhiteKey(88), "white")
assert.equal(blackOrWhiteKey(89), "white")
assert.equal(blackOrWhiteKey(92), "white")
assert.equal(blackOrWhiteKey(100), "black")
assert.equal(blackOrWhiteKey(111), "white")
assert.equal(blackOrWhiteKey(200), "black")
assert.equal(blackOrWhiteKey(2017), "white")
