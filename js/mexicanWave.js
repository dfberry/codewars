...
Task
   ...
   string into a Mexican Wave. You will be passed a string and you 
   must return that string in an array where an uppercase letter 
...
function wave(crowd){
  ...
  for(let i=0, max=crowd.length;i<max;i++){
    let person = crowd.split('');
    if(person[i]!= ' '){
      person[i] = person[i].toUpperCase();
    }
    waveCrowd.push(person.join(''));
  }
  return waveCrowd;
}
